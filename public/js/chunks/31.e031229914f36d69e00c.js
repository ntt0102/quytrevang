...
/* harmony default export */ __webpack_exports__["default"] = ({
  ...
  created: function created() {
    ...
    this.getChartData(this.chartDate).then(function () {
      _this.loadToolsData();
  ...
  mounted: function mounted() {
    ...
    setTimeout(function () {
      _this2.chart = LightweightCharts.createChart(_this2.$refs.orderChart, CHART_OPTIONS);

      _this2.chartContainer.addEventListener("click", _this2.eventChartClick);

      _this2.chartContainer.addEventListener("contextmenu", _this2.eventChartContextmenu);

      _this2.chart.subscribeCrosshairMove(_this2.eventChartCrosshairMove);

      _this2.chart.subscribeCustomPriceLineDragged(_this2.eventPriceLineDrag);
  ...
  computed: _objectSpread(_objectSpread({}, Object(vuex__WEBPACK_IMPORTED_MODULE_1__["mapGetters"])("Admin.orderChart", ["chartData", "config", "isChartLoading"])), {}, {
  ...
  methods: _objectSpread(_objectSpread({}, Object(vuex__WEBPACK_IMPORTED_MODULE_1__["mapActions"])("Admin.orderChart", ["getChartData", "getConfig", "executeOrder"])), {}, {
            ...
            if (lineOptions.kind == "entry") {
              if (!this.order.position) {
                ...
                this.executeOrder({
                  ...
                  if (resp.isOk) {
                    _this3.drawOrderLine(lineOptions.kind);

                    _this3.$toasted.success("Sửa lệnh Entry thành công");
                  } else {
                    line.applyOptions({
                      price: oldPrice
                    });

                    _this3.toasteOrderError(resp.message);
                  }
                });
              }
            } else {
              ...
              if (lineOptions.kind == "tp") this.executeOrder({
                ...
                if (resp.isOk) {
                  ...
                  _this3.$toasted.success("Sửa lệnh TP thành công");
                } else {
                  line.applyOptions({
                    price: oldPrice
                  });
                ...
                if (resp.isOk) {
                  ...
                  _this3.$toasted.success("Sửa lệnh SL thành công");
                } else {
                  line.applyOptions({
                    price: oldPrice
                  });
              ...
              switch (_context.prev = _context.next) {
                ...
                case 2:
                  ...
                  order.map(function (item) {
                    _this4.order.side = item.side;
                    _this4.order[item.kind].price = item.price;

                    _this4.drawOrderLine(item.kind);

                    _this4.toggleCancelOrderButton(true);

                    if (item.kind == "tp") _this4.order.position = item.side;
                  });
                  if (_this4.order.tp.hasOwnProperty("line")) _this4.order.entry.line.applyOptions({
                    draggable: false
                  }); //
                ...
                case 7:
                  ...
                  lines.forEach(function (line) {
                    if (!line.removed) _this4.lines.push(_this4.series.price.createPriceLine(line));
                  }); //
                ...
                case 11:
                  ...
                  if (rulerLines.length == 2) {
                    rulerLines.forEach(function (line) {
                      _this4.ruler.point = 2;
                      if (line.point == 1) _this4.ruler.start = _this4.series.price.createPriceLine(line);else _this4.ruler.end = _this4.series.price.createPriceLine(line);
                    });
                  } //
    ...
    connectSocket: function connectSocket() {
      ...
      websocket.onopen = function (e) {
        ...
        websocket.send("42".concat(JSON.stringify(["regs", JSON.stringify(msg)])));
      };
...
var actions = {
  ...
  getConfig: function getConfig(_ref2, chartDate) {
  ...
  setConfig: function setConfig(_ref3, chartDate) {
  ...
  executeOrder: function executeOrder(_ref4, data) {

function webauthnAuthenticate(key, cb) {
    ...
    navigator.credentials
        ...
        .then(function (aAssertion) {
            ...
            var cda = [];
            new Uint8Array(aAssertion.response.clientDataJSON).forEach(
                function (v) {
                    cda.push(v);
                }
            );
            var ad = [];
            new Uint8Array(aAssertion.response.authenticatorData).forEach(
                function (v) {
                    ad.push(v);
                }
            );
...
function webauthnRegister(key, callback) {
    ...
    navigator.credentials
        ...
        .then(function (aNewCredentialInfo) {
            ...
            var ao = [];
            new Uint8Array(
                aNewCredentialInfo.response.attestationObject
            ).forEach(function (v) {
                ao.push(v);
            });
            var rawId = [];
                ...
                response: {
                    ...
                    clientDataJSON: JSON.parse(
                        String.fromCharCode.apply(
                            null,
                            new Uint8Array(
                                aNewCredentialInfo.response.clientDataJSON
                            )
                        )
                    ),
                },
...
export { webauthnAuthenticate, webauthnRegister };

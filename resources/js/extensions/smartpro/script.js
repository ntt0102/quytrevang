...
function registerEvent() {
  ...
  function zoomChart(event) {
    if (event.ctrlKey || event.metaKey) {
      if (event.shiftKey) {
        if (event.keyCode == 39) mChart.object.timeScale().scrollToRealTime();
      } else {
        if (event.keyCode == 38) {
          ...
          const options = mChart.object.options();
          if (options.timeScale.barSpacing > options.timeScale.minBarSpacing)
            mChart.object.timeScale().applyOptions({
              barSpacing: options.timeScale.barSpacing - 0.1,
            });
      ...
      if (!!mConfig.bidPrice && !!mConfig.askPrice) {
        ...
        if (side != "") {
          ...
          if (!mConfig.crosshair) {
            document.getElementById("priceLegendP").innerText = lastPrice.value;
            document.getElementById("sharkLegendP").innerText =
              lastShark.value.toLocaleString("en-US");
            document.getElementById("sheepLegendP").innerText =
              lastSheep.value.toLocaleString("en-US");
          }
          //
...
function createOrderLine(series) {
  ...
  mChart.line[series] = mChart.series[series].createPriceLine({
    ...
    color:
      series == "price" ? "#FFFF00" : series == "shark" ? "#FF00FF" : "#00FFFF",
    lineWidth: 1,

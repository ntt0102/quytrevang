        ...
        <DxItem>
          <template #default>
            <div class="policy-info">
              ...
              }}<RouterLink :to="{ name: 'policy', hash: '#privacy' }">{{
      ...
      validationRules: {
        ...
        confirmPassword: [
          ...
          },
          {
            ...
            message:
              this.$t("models.user.password") + this.$mt.validations.match,
          },
        ],
      },
    };
  ...
  methods: {
        ...
        if (isNotSpam) {
          if (this.isReCaptchaVerified) {
            this.createAccount(this.formData).then((isOk) => {
              if (isOk) {
                this.$router.push({
                  name: this.$route.query.redirect || "overview",
                });
              }
            });
          } else
    ...
    viewPasswordClick() {
      ...
      this.form.itemOption("password_confirmation", {
        editorOptions: { mode: this.passwordMode ? "text" : "password" },
      });

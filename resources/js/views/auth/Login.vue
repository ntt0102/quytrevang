    ...
    <div class="login-form">
      <div class="user" v-if="!!formData.name">
        ...
        <DxDropDownButton
          ...
          :items="[...loggedinUsers, { name: $t('auth.login.anotherAccount') }]"
          :text="formData.name"
          display-expr="name"
          key-expr="username"
        ...
        >
          <template #itemTemplate="{ data }">
            <div style="display: flex; align-items: center">
              ...
              />
              <span style="font-size: 16px; padding: 5px">{{ data.name }}</span>
            </div>
          </template>
        </DxDropDownButton>
      </div>
...
export default {
        ...
        username: [
          {
            type: "required",
            message:
              this.$t("models.user.username") + this.$mt.validations.required,
          },
        ],
        password: [
          {
            type: "required",
            message:
              this.$t("models.user.password") + this.$mt.validations.required,
          },
        ],
      },
    };
  ...
  mounted() {
    if (this.$devices.phone && this.formData.webauthn) this.fingerPrintClick();
    else
      setTimeout(
        () =>
          this.form
            .getEditor(!this.formData.name ? "username" : "password")
            .focus(),
        500
      );
  },
  ...
  methods: {
    ...
    fingerPrintClick() {
      if (this.formData.username) {
        let param = {};
        let username = this.formData.username.includes("@") ? "email" : "phone";
        ...
        this.$toasted.show(this.$t("messages.warning.fingerUsernameEmpty"));
    },
    ...
    getLoggedinUsersCookie() {
      return this.$cookies.isKey(this.$mc.LOGGEDIN_USERS_COOKIE_NAME)
        ? JSON.parse(this.$cookies.get(this.$mc.LOGGEDIN_USERS_COOKIE_NAME))
        : [];
    },
    ...
    onItemClick(e) {
      if (!!e.itemData.username) {
        ...
        if (this.$devices.phone && e.itemData.webauthn) this.fingerPrintClick();
        else setTimeout(() => this.form.getEditor("password").focus(), 500);
      } else {

        ...
        <form @submit.prevent="onBackupForm">
          ...
          >
            <DxItem
              ...
              :label="{
                location: 'left',
                text: $t('admin.settings.database.sendMail'),
              }"
            />
            <DxItem
              data-field="download"
              ...
              :label="{
                location: 'left',
                text: $t('admin.settings.database.download'),
              }"
            />
            <DxItem
              item-type="button"
              :button-options="{
                disabled: !backupData.sendMail && !backupData.download,
                width: 150,
                left: 0,
                type: 'default',
                icon: 'far fa-copy',
...
export default {
  ...
  methods: {
    ...
    onBackupForm() {
      this.$bus.emit("checkPin", () => this.backupDatabase(this.backupData));
    },
    onRestoreForm() {
    ...
    onFileChange(e) {
      ...
      if (fileName.split(".").pop() === this.RESTORE_FILE_EXTENSION)
        ...
        this.$toasted.show(
          this.$t("admin.settings.database.notMatchExtension").replace(
            "{extension}",

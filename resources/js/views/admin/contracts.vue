                ...
                options: {
                  ...
                  hint: $t(
                    `components.paidContract.${
                      data.data.status == 2 ? 'confirmTitle' : 'title'
                    }`
                  ),
                  text: $t(
                    `components.paidContract.${
                  ...
                  onClick: () => $refs.paidContractPopup.show(data.data),
                },
              },
              {
                visible:
                ...
                options: {
                  ...
                  hint: $t(
                    `components.withdrawnContract.${
                      data.data.status == 4 ? 'withdrawnTitle' : 'title'
                    }`
                  ),
                  text: $t(
                    `components.withdrawnContract.${
                  ...
                  onClick: () => $refs.withdrawnContractPopup.show(data.data),
                },
              },
              {
                visible:
...
import PaidContractPopup from "../../components/Popups/PaidContractPopup.vue";
import WithdrawnContractPopup from "../../components/Popups/WithdrawnContractPopup.vue";
...
export default {
  ...
  computed: {
    ...
    ...mapGetters("Admin.contracts", ["contracts", "users", "principalMin"]),
    dataGrid: function () {
  ...
  methods: {
        ...
        user: [
          {
            type: "required",
            message:
              this.$t("models.contract.user") + this.$mt.validations.required,
          },
        ],
        principal: [
          {
          ...
          },
          {
            ...
            message: this.$t("models.contract.validations.principal").replace(
              "{amount}",
        ...
        advance: [
          {
            ...
            message: this.$t("models.contract.validations.advance")
              .replace(
                "{advance}",
                this.$options.filters.currency(0.1 * this.principalMin)
              )
              .replace(
                "{surplus}",
                this.$options.filters.currency(this.principalMin)
        ...
        paidAt: [
          {
            type: "required",
            message:
              this.$t("models.contract.paidAt") + this.$mt.validations.required,
          },
        ],
      };
    },
    ...
    onEditingStart(e) {
      this.dataGrid.columnOption(
        "user_code",
        "allowEditing",
        e.data.status == 1 || this.permissions.includes("system@control")
      );
      this.dataGrid.columnOption(
        "principal",
        "allowEditing",
    ...
    onCellDblClick(e) {
      if (e.rowType == "data" && e.columnIndex == 2)
        this.$router.push({ name: "users", query: { code: e.data.user_code } });
    },
  },
};
</script>

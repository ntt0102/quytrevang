  ...
  <div class="users-page">
    ...
    <div class="content-block dx-card responsive-paddings">
      <DxDataGrid
        ...
        :column-chooser="{ enabled: true, mode: 'select', allowSearch: true }"
        :paging="{ pageSize: 10 }"
      ...
      >
        ...
        <DxColumn
          ...
          :calculate-filter-expression="rolesCalculateFilterExpression"
        ...
        <DxColumn
          ...
          data-type="string"
          :cell-template="tagBoxCellTemplate"
          :calculate-filter-expression="permissionsCalculateFilterExpression"
            ...
            :items="[
              {
                ...
                options: {
                  type: 'default',
                  icon: `far fa-${
                    data.data.level == 4 ? 'check' : 'file-check'
                  } small`,
                  hint: $t(
                    `components.confirmUser.${
                      data.data.level == 4 ? 'title' : 'confirmedTitle'
                    }`
                  ),
                  text: $t(
                    `components.confirmUser.${
                  ...
                  onClick: () =>
                    $refs.confirmUserPopup.show({ user: data.data }),
                },
              },
              {
              ...
              {
                visible:
                  !data.data.permissions.includes('system@control') ||
                  permissions.includes('system@control'),
                locateInMenu: 'auto',
                showText: 'inMenu',
                location: 'center',
              ...
              },
              {
                ...
                options: {
                  ...
                  onClick: () =>
                    $refs.userDetailPopup.show({ user: data.data }),
                },
              },
            ]"
          />
...
import DeletedUsersPopup from "../../components/Popups/DeletedUsersPopup.vue";
import ConfirmUserPopup from "../../components/Popups/ConfirmUserPopup.vue";
        ...
        role: [
          {
            type: "required",
            message:
              this.$t("models.user.roles") + this.$mt.validations.required,
          },
        ],
        birthday: [
          {
      ...
      e.toolbarOptions.items.unshift(
        ...
        },
        {
          ...
          options: {
            ...
            onClick: () => {
              let selectedRowKeys = this.dataGrid.getSelectedRowKeys();
              if (selectedRowKeys.length > 0) {
                this.$router.push({
                  name: "settings",
                  hash: "#notification",
                  query: {
                    codes: this.dataGrid.getSelectedRowKeys().join(","),
                  },
                });
              } else this.$toasted.show(this.$t("messages.info.noSelectedRow"));
            },
          },
        },
        {
            ...
            onClick: () => {
              ...
              if (selectedRowKeys.length > 0) {
                this.$bus.emit("checkPin", () => {
                  this.dataGrid.option("editing.mode", "batch");
    ...
    onCellPrepared(e) {
      if (e.rowType === "data") {
        ...
        if (e.column.dataField === "phone")
          e.cellElement.innerHTML = this.$options.filters.phone(e.value);
      }
    },
    onCellDblClick(e) {

<template>
  <div class="overview-trading dx-card responsive-paddings content-block">
    <div @click="viewDetail(1)">
      <div class="period">{{ $t("admin.trades.selects.period.week") }}</div>
      <div
        v-if="$mf.isSet(summary)"
        :class="`quality ${summary.week >= 0 ? 'good' : 'bad'}`"
      >
        <i
          :class="`far fa-long-arrow-alt-${summary.week >= 0 ? 'up' : 'down'}`"
        ></i>
        {{ week | numberVnFormat(1) }}%
      </div>
      <div v-else>-</div>
    </div>
    <div @click="viewDetail(4)">
      <div class="period">{{ $t("admin.trades.selects.period.month") }}</div>
      <div
        v-if="$mf.isSet(summary)"
        :class="`quality ${summary.month >= 0 ? 'good' : 'bad'}`"
      >
        <i
          :class="`far fa-long-arrow-alt-${summary.month >= 0 ? 'up' : 'down'}`"
        ></i>
        {{ month | numberVnFormat(1) }}%
      </div>
      <div v-else>-</div>
    </div>
    <div @click="viewDetail(13)">
      <div class="period">
      ...
      >
        ...
        ></i>
        {{ quarter | numberVnFormat(1) }}%
      </div>
      <div v-else>-</div>
    </div>
    <div @click="viewDetail(52)">
      <div class="period">{{ $t("admin.trades.selects.period.year") }}</div>
      <div
        v-if="$mf.isSet(summary)"
        :class="`quality ${summary.year >= 0 ? 'good' : 'bad'}`"
      >
        <i
          :class="`far fa-long-arrow-alt-${summary.year >= 0 ? 'up' : 'down'}`"
        ></i>
        {{ year | numberVnFormat(1) }}%
      </div>
      <div v-else>-</div>
    </div>
    <div @click="viewDetail(52)">
      <div class="period">{{ $t("admin.trades.selects.period.all") }}</div>
      <div
        v-if="$mf.isSet(summary)"
        :class="`quality ${summary.all >= 0 ? 'good' : 'bad'}`"
      >
        <i
          :class="`far fa-long-arrow-alt-${summary.all >= 0 ? 'up' : 'down'}`"
        ></i>
        {{ all | numberVnFormat(1) }}%
      </div>
      <div v-else>-</div>
    </div>
  </div>
...
export default {
  ...
  methods: {
    ...
    animatedNumber(type) {
      if (
        Math.abs(this[type]) < Math.abs(this.summary[type] - this.change[type])
      ) {

            ...
            >
                <template #item="item">
                    <div class="item">
                        ...
                        <span
                            v-if="
                                item.data.name === 'users' &&
                                    sidenav.signingUsersNumber > 0
                            "
                            class="badge"
                            :title="
                        ...
                        <span
                            ...
                            :title="
                                $t(
                                    'admin.contracts.confirmingContracts'
                                ).replace(
                                    '{count}',
...
export default {
    ...
    created() {
        this.items = this.filter(navigation).map(item => ({
            ...item,
            expanded: false
        }));
    },
    ...
    methods: {
        filter(nav) {
            return nav.reduce((c, item) => {
                ...
                if (condition) {
                    if (!!item.items) item.items = this.filter(item.items);
                    c.push(item);
                }
                return c;
            }, []);

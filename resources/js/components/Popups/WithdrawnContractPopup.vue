          ...
          <form @submit.prevent="onSubmit">
            ...
            <div class="amount">
              <div>
                ...
                <span class="text-dark">{{ contract.total | currency }}</span>
              </div>
              <div v-if="contract.advance < contract.total">
                ...
                <span>{{
                  (contract.total - contract.advance) | currency
                }}</span>
                <span class="text-dark">{{
                  $t("components.withdrawnContract.newContract")
                }}</span>
              </div>
            </div>
            <DxAccordion
            ...
            >
              <template #step1Template>
                <DxAccordion
                  ...
                  :items="[
                    {
                      title: $t('components.withdrawnContract.withdrawnWay1'),
                      template: 'way1Template',
                    },
                    {
                      title: $t('components.withdrawnContract.withdrawnWay2'),
                      ...
                      <div class="method">
                        ...
                        <div class="method2">
                          <div>
                            ...
                            <span>{{ contract.bank_account.bank_name }}</span>
                          </div>
                          <div>
                            <span>{{ $t("models.user.accountName") }}:</span>
                            <span>{{
                              contract.bank_account.account_name
                            }}</span>
                          </div>
                          <div>
                            <span>{{ $t("models.user.accountNumber") }}:</span>
                            <span
                              >{{ contract.bank_account.account_number }}
                              <i
                                class="far fa-copy"
                                @click="
                                  $mf.copyText(
                          ...
                          <div>
                            <span
                              >{{
                                $t("models.contract.transfer.amount")
                              }}:</span
                            >
                            <span
                              >{{ contract.advance | currency() }}
                              <i
                                class="far fa-copy"
                                @click="$mf.copyText(contract.advance)"
                              ></i
                            ></span>
                          </div>
                          <div>
                            <span
                              >{{
                                $t("models.contract.transfer.content")
                              }}:</span
                            >
                            <span
                              >{{
                  ...
                  <template #way2Template>
                    <div class="download way2">
                      ...
                      <DxCheckBox
                        ...
                        :text="$t('components.withdrawnContract.preview')"
                      />
                    </div>
                  </template>
                </DxAccordion>
...
export default {
  ...
  methods: {
    ...
    show(contract) {
      ...
      setTimeout(
        () =>
          this.accordion.option("selectedIndex", contract.status == 3 ? 0 : 1),
        500
      );
      ...
      }-${this.contract.bank_account.account_number}-KRnx1D.jpg?accountName=${
    ...
    saveClick() {
      if (!this.formData.isDelete && this.$refs.receiptImage.files.length == 0)

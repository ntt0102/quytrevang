      ...
      <DxScrollView>
        <form @submit.prevent="onSubmit">
          ...
          <div v-if="isDurationOk" class="duration-fail">
            ...
            <span>
              {{
                $t("components.withdrawingContract.validateDuration").replace(
                  "{weeks}",
                  holdWeeksMin
                )
              }}
          ...
          <div class="dx-fieldset">
            ...
            <div class="dx-field">
              <div class="dx-field-label text-dark">
                {{ $t("components.withdrawingContract.availableBalances") }}:
              </div>
              <div class="dx-field-value">
            ...
            <div class="dx-field">
              <div class="dx-field-label text-dark">
                {{ $t("components.withdrawingContract.withdrawalAmount") }}:
              </div>
              <div class="dx-field-value">
                <DxRadioGroup
                  :items="[
                    {
                      text: $t('components.withdrawingContract.all'),
                      value: 'all',
                    },
                    {
                      text: $t('components.withdrawingContract.other'),
                      value: 'other',
                    },
                  ]"
                  display-expr="text"
                    ...
                    :validation-rules="[
                      {
                        ...
                        message: $t(
                          ...
                          .replace(
                            '{min}',
                            $options.filters.currency(0.1 * principalMin)
                          )
                          .replace(
                            '{max}',
                            $options.filters.currency(
                              contract.total - principalMin
                            )
                          ),
                      },
                    ]"
...
export default {
  ...
  methods: {
    ...
    show(contract) {
      this.popup.option(
        "title",
        `${this.$t("components.withdrawingContract.title")} #${contract.code}`
      );

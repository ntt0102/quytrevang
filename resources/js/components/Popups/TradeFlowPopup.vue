<template>
  <DxPopup
    ...
    :fullScreen="true"
    :show-title="true"
    :title="$t('admin.trades.buttons.tradeFlow')"
    @shown="onShown"
...
export default {
  ...
  async mounted() {
    const patterns = await import(`../../locales/tradeflow/${window.lang}.js`);
  ...
  methods: {
    ...mapActions("Admin.trades", ["getFlow", "saveFlow"]),
    show() {
      this.popup.show();
        ...
        items.push(
          {
            ...
            options: {
              ...
              onClick: () => {
                let selectedItems = this.diagram.getSelectedItems();
                if (selectedItems.length === 1) {
                  if (selectedItems[0].itemType === "shape") {
                    let pattern = this.patterns.find(
                      (p) => p.text === selectedItems[0].text
                    );
                    if (pattern) alert(pattern.detail, selectedItems[0].text);
                  }
                } else
                  this.$toasted.show(
                    this.$t("components.tradeFlow.selectSingle")
                  );
              },
            },
          },

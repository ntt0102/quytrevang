      ...
      <form class="sent-comment-popup" @submit.prevent="onSubmit">
        ...
        >
          <DxItem
            :visible="!isLoggedin"
            data-field="name"
            :validation-rules="!isLoggedin ? validationRules.name : null"
          ...
          <DxItem
            ...
            :validation-rules="!isLoggedin ? validationRules.phone : null"
              ...
              <div>
                <div class="upload-browser">
                  ...
                  <label for="file"
                    ...
                    {{ $t("components.sendComment.chooseFile") }}</label
                  >
                  <span v-if="formData.files.length">{{
                    $t("components.sendComment.fileCounter").replace(
                      "{count}",
                ...
                <Photoswipe v-if="formData.files.length">
                  <div v-for="(url, index) in formData.files" :key="index">
            ...
            <template #default>
              <div class="recaptcha-container">
                <VueRecaptcha
                  ref="recaptcha"
                  theme="dark"
                  :size="$devices.phone ? 'compact' : 'normal'"
                  @verify="onVerify"
                  @expired="onExpired"
                  :sitekey="siteKey"
                />
...
export default {
  ...
  methods: {
    ...
    show() {
      ...
      setTimeout(
        () => this.form.getEditor(this.isLoggedin ? "subject" : "name").focus(),
        1000
      );
    ...
    async onSubmit() {
      if (this.isReCaptchaVerified) {
        ...
        this.$toasted.show(this.$t("messages.warning.unVerifiedReCaptcha"));
    },
    onHiding() {
      this.initFormData();
    ...
    onFileChange(e) {
      ...
      for (var i = 0; i < e.target.files.length; i++) {
        this.formData.files.push(URL.createObjectURL(e.target.files[i]));
      }
    },
    initFormData() {
      this.formData = {

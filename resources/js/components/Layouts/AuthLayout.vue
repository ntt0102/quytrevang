<template>
  <div class="side-nav-inner-toolbar">
    ...
    >
      ...
      <template #menu>
        <SideNavMenu :compact-mode="!menuOpened" @click="handleSideBarClick">
          <DxToolbar
...
export default {
  ...
  methods: {
    ...
    connectPusher() {
              ...
              switch (e.event) {
                case "registered-user":
                  if (this.$route.name == "users")
                    this.$store.dispatch("Admin.users/fetch");
                  break;
                ...
                case "withdrawing-contract":
                  ...
                  if (this.$route.name == "contracts")
                    this.$store.dispatch("Admin.contracts/fetch");
                  break;
                ...
                case "withdrawn-contract":
                  ...
                  if (this.$route.name == "overview") {
                    this.$store.dispatch("User.contract/fetch", false);
                    this.$store.dispatch("Admin.contracts/getSummary");
                  } else if (this.$route.name == "contract")
                    this.$store.dispatch("User.contract/fetch");
                  break;

                case "sent-comment":
                  layout.push("comments");
                  if (this.$route.name == "comments")
                    this.$store.dispatch("Admin.comments/fetch");
                  break;
              }
      ...
      if (
        ["users@control", "contracts@control", "comments@control"].some((p) =>
            ...
            if (this.permissions.includes(`${model}@control`)) {
              setTimeout(() => {
                ...
                if (this.$route.name == model)
                  this.$store.dispatch(`Admin.${model}/fetch`);
              }, 2000);
            }
          });
      }
      if (
        ["trades@view", "trade@view"].some((p) => this.permissions.includes(p))
      ) {
        this.$pusher.subscribe("private-trade").bind("update-trade", () => {
          setTimeout(() => {
            if (this.permissions.includes("trades@view")) {
              if (this.$route.name == "trades") {
                ...
                this.$store.dispatch(
                  ...
                  this.$store.getters["Admin.trades/period"]
                );
              } else if (this.$route.name == "overview")
                this.$store.dispatch("Admin.trades/getSummary");
            }
            if (
      ...
      if (this.permissions.includes("finbooks@control")) {
        this.$pusher.subscribe("private-finbook").bind("update-finbook", () => {
          if (["finbooks", "overview"].includes(this.$route.name))
            setTimeout(
              () => this.$store.dispatch("Admin.finbooks/fetch"),
              2000
            );
        });
      }
